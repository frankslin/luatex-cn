...
修复：
- `\抬头`／`\平抬`／`\单抬` 之后的夹注跳到列首、落进天头 — 抬头是通过 `ATTR_INDENT` 记的绝对行位，属性一直留到组结束；字形每次换列都会按抬头作用域重新解析，夹注（textflow）却直接读原始属性，于是抬头行自己的夹注（「聖祖仁皇帝……臣南懐仁製」的「臣」）跳回行首，同一块里后面每一列的夹注也跟着跳。现在把属性交给 textflow 之前先判断是不是本夹注自己的抬头：已越出抬头列、或抬头列的缩进已被前面的正文用掉，就清掉。夹注**内部**的 `\单抬` 不受影响，照旧凸进天头
- 同一页上连续两个 `正文` 环境，第二块整块压印在第一块上（版心同时印出「一」和「二」），并伴随 `You can't use \prevdepth in horizontal mode` 等三个报错 — `output_pages` 只在自己产出的页与页之间插分页 penalty，最后一页不收尾，而每页都是高度为 0、按绝对坐标定位的 `\vbox`，页面于是永远装得下下一块；现在 `core.process` 记住上一块末页占掉的 `band:col` 格子，与下一块首页的占格相交时才补分页 penalty —— `分栏` + `\换栏` 把两块错开到同页不同栏的双栏对照版式照旧共用一页。另把 `正文`/`\Content` 开头的 `\nointerlineskip` 移到 `\par` 之后（`\prevdepth` 赋值只在竖直模式下合法）
- `\pageSetup{页码样式=digits}` 对版心页码无效 — 版心渲染一直写死用位值中文数字（144 → 一百四十四），从不读这个设置，只有 vbook 的竖排页码读它；现在两条路径共用 `utils.format_page_number`，digits / chinese / arabic / none 一致生效。`页码样式` 的初始值同时由 `none` 改为空串以区分「未设置」，否则任何一次 `\pageSetup{纸宽=...}` 都会顺带关掉版心页码 (#161)
- 横排：空格窄于 1/4 em 的西文字体（Source Sans 3 的空格是 0.2 em）排出的纯西文行压不回去，整词溢出版心（`Overfull \hbox`）— 西文词距的 clreq 界限「最小挤到 1/4 em」实现成 `min(w, 0.25em)` 再截 shrink，空格本就低于下限时下限等于自身宽度，shrink 被截成 0，整行没有任何可挤压量。上限那边同理：空格宽过 1/2 em 的字体 stretch 被清空。现在界限只收窄字体自带的弹性，自然宽度已在界限之外的空格原样保留
- 横排：`1/4 em`、`a/b` 这类纯西文串里，分隔号 `/` 左右各被加了一个 1/4em 中西间距 —— `/` `-` `·` `–` 登记在标点表里、`kind` 为 `cjk_punct`，横排后端又把 `cjk_punct` 一律当作「中文一侧」自行判断边界，于是整行没有一个汉字也照加。这几个字面本就是西文自带的半角标点（中文另有满幅的 `／` `～` `‧`），现由标点表的 `western_form` 标出，不构成中西边界的中文一侧；破折号、省略号这些只有中文用的满幅标点仍是中文的一部分，与西文相邻照加 1/4em。判断权一并收回共享层的 `cjk_western.takes_spacing`（HR5：clreq 规则只写在 shared/），竖排后端一直用的就是它——横排后端此前重写了一份且结论相反，同一段文字横竖排间距因此不一致
- 四库全书彩色模板下句读符号被画成正文色 — 文档级样式（`\documentSetup{font-color=...}`）通过全局 TeX 属性贴到所有新建节点上，句读装饰标记继承后，其样式在渲染时优先于装饰注册表，顶掉了句读自己的颜色；标记现在改挂自己的样式 id (#157)
- `\句读设置{句读颜色=...}` 之前完全无效（装饰样式写死 red）— 现按设置的颜色注册，颜色变更时重新注册 (#157)
...
变更：
- 新增 \中横排 / \CombineUpright 命令（clreq 直排中西混排配置之「横排入一个字格」，即縦中横，P3 第三步）— 两三位数字/极短西文横排挤进一个字幅：整组共占一格（layout 让组内其余字形字幅与字距归零，组天然不可拆、禁则拉字时整组同行）、组内基线同高横向排开、总宽超过 1em 时只做横向压缩（字高与笔画竖向粗细不变，同 JIS 縦中横通行做法）、竖向按字体升降部把基线居中于格；与前后汉字照常有 1/4em 中西间距。组号递增，相邻两组不误并。适合日期「\中横排{12}月」、卷次等；较长词句用 \横置。断言 6 条含负对照（跨组步长 2.5em vs 直立入格 3.6em）；回归用例 combine-upright.tex 含直立对照行。直排中西混排三种配置（直立/旋转/中横排）至此齐备
