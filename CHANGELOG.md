...
新增：
- 横排 clreq 模式 luatex-cn-hori — 中西间距 1/4em（可挤至 1/8、拉至 1/2，含 clreq 规定的三类例外）、标点宽度调整与大陆式/台式/不调整三套预设、引号体例转换（默认不改来稿）、四级换行禁则与符号分离禁则、按 clreq 优先顺序的行内二次分配（挤压 7 级 / 拉伸 2 级 + 兜底均分）、行末标点半字宽挤压、行尾点号悬挂、均排 (#127, #129, #130, #131)
...
- 大陆式字面分布改为度量驱动 — 不再「经验偏移 + 部分字体补偿」，而是读字形 boundingbox 把**墨迹**锚到字幅内的规范位置：句号墨心 (0.857, 0.55)em、逗号顿号 (0.857, 0.31)em（偏靠右上、贴前字），中点类（：；？！）只做横向偏靠（0.857）、纵向随字形设计；锚点值为 TW-Kai 在旧实现下的实测落点（与旧基线差 ≤0.001em），同一锚点对所有字体成立。修复京華老宋体等无 vert 特性字体下逗号句号「前后都有空白、横向脱靶」的问题。竖排 clreq 断言新增「大陆式偏靠」守卫（Tm 右移 >0.2em，居中即回归）。无 boundingbox 时退回原经验偏移路径。仅影响 punct-style=mainland 的竖排文档（ltc-cn-vbook 系），ltc-guji 默认 taiwan 不受影响
