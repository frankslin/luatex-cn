...
新增：
- clreq 符合度声明矩阵 docs/CLREQ-CONFORMANCE.md — 逐条对 clreq TR 章节编号（2.1.1–7.1.3、附录 A），横排 luatex-cn-hori 与竖排 vbook 双列状态，证据指向断言组/单测/回归用例；含 5 条有意偏离（禁则词典序比价、悬挂默认关、字距兜底、横排夹注号不锚定、ltc-guji 整类不宣称）与 3 条不实现及理由（bidi、儿化小字、表单章）、复核命令与维护约定
