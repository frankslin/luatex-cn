...
本文档整理中国大陆和台湾在竖排排版中对标点符号的处理标准，作为 luatex-cn 标点模块实现的参考依据。
...
2. **Layout 阶段**: 挤压调整（中国大陆模式：半角挤压；台湾模式：不挤压）
3. **Render 阶段**: 位置偏移（中国大陆模式：偏右/偏右上；台湾模式：居中无偏移）
