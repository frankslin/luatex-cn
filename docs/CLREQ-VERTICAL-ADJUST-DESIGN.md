...
- ~~`adjust.solve` 的 `deficit > 0` 只报告不决策~~ → **已补**：
  ...
  **⚠️ 词典序不等于 clreq 字面——这是一处有意偏离，须如实声明。**
  clreq 的原话是「最后没有挤压机会，再从前一行取最后一个字」，即**只要挤
  得进就该挤进**；词典序求的是「尽量少动最后手段」。两者在一种情形下分歧：
  挤进可行、但要把字距压得比推出拉得更狠时，词典序选推出。实测「两侧都有
  标点空白」的列——挤进要把字距从 0.1em 压到 0.048em（−52%），推出只需
  拉到 0.132em（+32%），照规范字面该挤进。

  取舍理由：−52% 的字距在竖排密排里非常刺眼，而 +32% 的拉伸尚在观感可接受
  范围。**这是排版质量优先于规范字面的选择，不是实现不到位**，将来的
  `docs/CLREQ-CONFORMANCE.md` 要把「禁则的解决方式」标为
  **部分实现（有意偏离）** 并附此理由，不能标成完全符合。

  **容差不能硬编码**：`adjust.solve` 声明「不假设单位」（调用方传 em 比值
  或 sp 均可，1 em = 655360 sp）。比较形变时若写死绝对容差，sp 量纲下任何
  ≥1 sp 的差异都会分出胜负，clreq 明文的「全等 → 先挤进」几乎永不触发，
  决策实际由浮点舍入决定（1 sp ≈ 0.0000002 英寸，两个方案视觉完全一致）。
  现在容差由后端按量纲显式传入（竖排传 `grid_height × 0.001`），共享层缺省
  按输入规模自适应。单测里 em 与 sp 两种量纲都有用例——只用 em 测是看不见
  这个问题的。
