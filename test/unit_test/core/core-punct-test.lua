...
test_utils.run_test("flatten: 句号在结束夹注符号前收回末端半字（始端为 0）", function()
    ...
    test_utils.assert_eq(r[2].head, 0, "中国大陆式点号的空白在末端，始端收回必须为 0")
end)
...
test_utils.run_test("anchors: 中国大陆式直排——墨心挪到偏靠锚点", function()
...
test_utils.run_test("anchors: 中国大陆式直排是偏靠不是居中（回归守卫）", function()
    ...
    for _, c in ipairs({ 0xFF0C, 0x3001, 0x3002, 0xFF1A, 0xFF1B, 0xFF01, 0xFF1F }) do
        ...
        test_utils.assert_true(a.x > 0.7,
            string.format("U+%04X 直排中国大陆锚点应偏右", c))
    end
    -- 句号在右上：y 锚点高于逗号
